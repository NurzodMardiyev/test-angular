...
@Component({
  ...
  templateUrl: './devidedform.html',
  styleUrl: './devidedform.scss',
  providers: [MessageService],
})
export class Devidedform implements OnInit {
  ...
  initializeForm() {
    this.paymentForm = this.fb.group({
      ...
      arizachiNomi: ['', [Validators.required, Validators.maxLength(100), Validators.pattern(/^[a-zA-Z0-9\s]+$/)]],
      stir: ['', [Validators.required, Validators.pattern(/^\d{9,15}$/)]],
      telefonRaqami: ['', [Validators.required, Validators.pattern(/^\+?\d{9,15}$/)]],
